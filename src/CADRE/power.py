''' Adaptation of simplified power discipline for LS2 based on CADRE '''
...
class Power_SolarPower(Component):
    ...
    def __init__(self, n=2):
		...
		#Constants
		#LS2 solar array efficiency
		eff = 0.17
		
		#area reduction coefficient to account for different soler cell size on LS2 compared to CADRE
		red = (0.0116/7)/0.0027
    ...
    def solve_nonlinear(self, params, unknowns, resids):
        ...
        A = params['exposedArea']
        A_tot = np.zeros((n, ))   
	    for i in range(n):
		    A_tot[i] = np.sum(A[:, :7, i])
			
        unknowns['P_sol'] = A_tot*P_sun*LOS*eff*red

    def linearize(self, params, unknowns, resids):
        """ Jacobian"""
        J = {}

        J['P_sol','LOS'] = np.ones((n, ))*A_tot*P_sun*eff*red
        J['P_sol','exposedArea'] = np.ones((7, 12, n))*P_sun*LOS*eff*red
